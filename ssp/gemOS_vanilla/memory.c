...
static u64 get_free_pages(struct page_list *pgl,u32 num)
{
  u64 pfn;
  u32 bit;
  u64 first_pfn = 0;
...
static u64 get_free_page(struct page_list *pgl)
{
  u64 pfn;
  u32 bit;
  u32 size_in_u32 = pgl->size >> 5, once = 0; //changed to 5 from 8
  ...
  //printk("size_in_u32:%u,last_free_pos:%u\n",size_in_u32,pgl->last_free_pos);
  while(pgl->last_free_pos < size_in_u32){
      if(*scanner == 0xffffffff){
         found = 0;
         pgl->last_free_pos++;
...
u64 os_pfns_alloc(u32 region, u32 num)
{
    u64 pfn;
    u32 loop_index = 0;
    struct page_list *pgl;
...
void *os_page_alloc_pages(u32 region, u32 num){
   ...
   u64 pfn = os_pfns_alloc(region, num);
...
void os_pages_free(u32 region, void *paddress, u32 num)
{
    ...
    u64 pfn = ((unsigned long)paddress - pgl->start_address) >> PAGE_SHIFT;
    u32 loop_index = 0;
