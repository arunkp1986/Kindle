...
static void init_pagelist(int type, u64 start, u64 end)
{
     ...
     pgl->last_free_pos = 0;
     bytes_page_bitmap = (pgl->size >> 3)?(pgl->size >> 3):1; //bytes requires to save bitmap
...
static void init_pfn_info_alloc_memory(){
    ...
    u32 num_pages;
    struct page_list *pgl= &pglists[OS_PT_REG];

    //startpfn = os_pfn_alloc(OS_PT_REG);
    startpfn = get_free_page(pgl);
    u64 addr = startpfn << PAGE_SHIFT;
    ...
    for(i=0; i< num_pages; i++){
        ...
        currentpfn = get_free_page(pgl);
        addr = currentpfn << PAGE_SHIFT;
        bzero((char *)addr, PAGE_SIZE);
...
static u32 get_free_pages(struct page_list *pgl,u32 num)
{
  ...
  if((found != num) && pgl->last_free_pos >= size_in_u32){
        pgl->last_free_pos = 0;
	start_free_pos = 0;
	found = 0;
...
static u32 get_free_page(struct page_list *pgl)
{
  ...
  if(!found && pgl->last_free_pos >= size_in_u32){
      pgl->last_free_pos = 0;
      scanner = ((u32 *)(pgl->bitmap)) + pgl->last_free_pos;
      while(pgl->last_free_pos < initial_free_pos){
          if(*scanner == 0xffffffff){
...
{  
   ...
   if(address >= NVM_USER_REG_START && address < REGION_NVM_ENDMEM){
	   region = NVM_USER_REG;
   }
   if(address >= NVM_META_REG_START && address < NVM_USER_REG_MAGIC){
	   region = NVM_META_REG;
   }
   os_page_free(region, (void *) (address));
   reset_pfn_info(pfn);
   if(region == USER_REG){
       stats->user_reg_pages--;
   }
...
void os_pfns_free(u32 region, u64 pfn, u32 num)
{
   ...
   os_pages_free(region, (void *) (address), num);
