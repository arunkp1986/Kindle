...
int kmain(unsigned long magic)
{
         ...
         *pdpte = (*pdpte) & 0xfffffffffffffff3UL;
         ...
         /*asm volatile("sti;"
                      :::"memory");*/
         init_swapper(); 
         stats = os_alloc(sizeof(struct os_stats),OS_DS_REG); 
         bzero((char *) stats, sizeof(struct os_stats));

         config = os_alloc(sizeof(struct os_configs),OS_DS_REG); 
         bzero((char *) config, sizeof(struct os_configs));
         config->apic_tick_interval = 32;
         if(bdev_init())
		 printk("Disk initialization Error!\n");
	 //char *buf = osmap(os_pfn_alloc(OS_PT_REG));
	 //bdev_read_block(buf, 0);
	 //struct data *dt = (struct data*)buf;
         //printk("Interval: %u, offset:%u, ops:%c, size:%d, type:%d \n", dt->interval, dt->offset, dt->ops, dt->size, dt->type);
