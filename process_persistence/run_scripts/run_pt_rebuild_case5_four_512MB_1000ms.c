...
int main(int argc, char* argv[]) {
   ...
   int pid = fork();
   if(!pid){
       strcpy(cmd,"./pt_rebuild_case5_four_512MB_1000ms_run.sh");
       system(cmd);
   }
   else{
       sleep(10);
	   ...
	   printf("child\n");
	      ...
	      exit(0);
           ...
           strcpy(cmd,"port=$(cat rebuild_case5_four_512MB_1000ms.out| grep 'connections on port'| cut -f7 -d ' ');./run.except $port");
           system(cmd);
       }
       else{
