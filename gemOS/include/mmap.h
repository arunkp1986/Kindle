...
#define MMAP_AREA_END  (MMAP_START+0x67DE00000)
...
extern struct vm_area* create_vm_area(u64 start_addr, u64 end_addr, u32 flags, u8 is_nvm);

static struct vm_area * alloc_vm_area()
{
    //struct vm_area *vm = os_alloc(sizeof(struct vm_area),OS_DS_REG);
    struct vm_area *vm = (struct vm_area *)os_chunk_alloc(sizeof(struct vm_area),OS_DS_REG);
    stats->num_vm_area++;
    return vm;
}
...
{
    stats->num_vm_area--;
    //os_free(vm, sizeof(struct vm_area),OS_DS_REG);
    os_chunk_free(OS_DS_REG,(u64)vm, sizeof(struct vm_area));
}
