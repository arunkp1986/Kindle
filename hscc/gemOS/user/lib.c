...
int read_blk(void *addr, u32 num)
{
    return (int)_syscall2(SYSCALL_BLK_READ, (u64)addr, num);
}
...
unsigned checkpoint_start()
{
  return(_syscall0(SYSCALL_START_TRACK));
}

unsigned checkpoint_end()
{
  return(_syscall0(SYSCALL_END_TRACK));
}

unsigned checkpoint_end2()
{
  return(_syscall0(SYSCALL_END_TRACK2));
}


unsigned gem5_dump_stats()
{
  return(_syscall0(SYSCALL_DUMP_GEM5));
}

unsigned gem5_reset_stats()
{
  return(_syscall0(SYSCALL_RESET_GEM5));
}


unsigned checkpoint_stats()
{
  return(_syscall0(SYSCALL_CTX_PRINT));
}
unsigned page_migrate()
{
  return(_syscall0(SYSCALL_MIGRATE));
}
...
void* umemset(void *s, int c, u32 n){
