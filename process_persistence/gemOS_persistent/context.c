...
int install_os_pts_range(struct exec_context *ctx, u64 start, u64 length){
	...
	//printk("entry:%x\n",entry);
	    ...
	    pfn = os_pfn_alloc(NVM_META_REG);
            *entry = (pfn << PAGE_SHIFT) | 0x3;
	    clflush_multiline((u64)entry, sizeof(u64));
...
u32 invalidate_pte(struct exec_context *ctx, unsigned long addr)
{
   ...
   *pte_entry = 0;
   clflush_multiline((u64)pte_entry, sizeof(u64)); 
...
u32 map_physical_page(unsigned long base, u64 address, u32 access_flags, u32 upfn, u8 is_nvm)
{
   ...
   if(!*ptep)
   {
      ...
      *ptep = (pfn << PAGE_SHIFT) | 0x7;
      clflush_multiline((u64)ptep, sizeof(unsigned long)); 
      os_addr = osmap(pfn);
...
int exec_init(struct exec_context *ctx, struct init_args *args)
{
   ...
   textpfn = install_ptable_multi((u64) os_addr, ctx->mms[MM_SEG_CODE].start, CODE_PAGES, 0,1);//allocated in nvm
   ctx->mms[MM_SEG_CODE].next_free = ctx->mms[MM_SEG_CODE].start + (CODE_PAGES << PAGE_SHIFT);

   install_ptable((u64) os_addr, &ctx->mms[MM_SEG_DATA], 0, 0, 1); //allocates in nvm   
   ...
   install_ptable((u64) os_addr, &ctx->mms[MM_SEG_STACK], 0, 0, 1); //allocates in nvm
...
struct exec_context *get_ctx_by_name(char* name)
{
   struct exec_context * ctx;
   for(int pid=0; pid<MAX_PROCESSES; pid++){
       ctx = (struct exec_context *)(ctx_list + pid);
...
void init_swapper()
{
   ...
   //swapper->files[0] = create_standard_IO(STDIN);
   //swapper->files[1] = create_standard_IO(STDOUT);
   //swapper->files[2] = create_standard_IO(STDERR);
...
void setup_child_context(struct exec_context *child)
{
   ...
   //child->state = WAITING;  // Will be eligible in next tick
   stats->num_processes++;
}
