...
long look_up_hint_addr(struct vm_area* vm, u64 addr, int length, int prot, int flags, u8 is_nvm)
{
    ...
    while(vm)
    {
         //printk("vm[%x -> %x -> %x]\n",vm->vm_start, vm->vm_end, vm->vm_next);
        ...
        } else
        {
            ...
            if(addr >= start_page && addr < end_page)
            {
                ...
                //printk("vm[%x -> %x -> %x]addr [%x] available[%d] requested[%d] \n",vm->vm_start, vm->vm_end, vm->vm_next, addr, available_pages, required_pages);
...
static struct vm_area* try_merge_vms(struct vm_area *vm, struct vm_area *prev, u8 check_next)
{
   ...
   if(prev && vm->access_flags == prev->access_flags && vm->vm_start == prev->vm_end){
             ...
             if(current->persistent){
                  ...
                  log1->event = VMA_CHANGE;
                  log1->address.index = get_vma_index(current,prev);
                  log1->address.offset = 0;
                  log1->payload = (void*)os_chunk_alloc(sizeof(struct vm_area),NVM_META_REG);
                  if(!log1->payload){
                      printk("os chunk alloc failed at %s\n",__func__);
		  }
                  memcpy((char*)log1->payload, (char*)prev, sizeof(struct vm_area));
                  log1->size = sizeof(struct vm_area);
                  list_add_tail(&log1->list, &cs->log);
                  clflush_multiline((u64)log1->payload, sizeof(struct vm_area));
                  clflush_multiline((u64)log1, sizeof(struct log_entry));
                  clflush_multiline((u64)&cs->log, sizeof(struct list_head));
   ...
   if(check_next && next && next->access_flags == vm->access_flags && next->vm_start == vm->vm_end){ //keep vm, free next
             ...
             vm->vm_end = next->vm_end;
             if(current->persistent){
                  dprintk("creating VMA_CHANGE log3 at %s\n",__func__);
                  struct log_entry* log3 = (struct log_entry*)os_chunk_alloc(sizeof(struct log_entry),NVM_META_REG);
                  if(!log3){
                      printk("os chunk alloc failed at %s\n",__func__);
		  }
                  log3->event = VMA_CHANGE;
                  log3->address.index = get_vma_index(current,vm);
                  log3->address.offset = 0;
                  log3->payload = (void*)os_chunk_alloc(sizeof(struct vm_area),NVM_META_REG);
                  if(!log3->payload){
                      printk("os chunk alloc failed at %s\n",__func__);
		  }
                  memcpy((char*)log3->payload, (char*)vm, sizeof(struct vm_area));
                  log3->size = sizeof(struct vm_area);
                  list_add_tail(&log3->list, &cs->log);
                  clflush_multiline((u64)log3->payload, sizeof(struct vm_area));
                  clflush_multiline((u64)log3, sizeof(struct log_entry));
                  clflush_multiline((u64)&cs->log, sizeof(struct list_head));

		  dprintk("creating VMA_REMOVE log4 at %s\n",__func__);
                  struct log_entry* log4 = (struct log_entry*)os_chunk_alloc(sizeof(struct log_entry),NVM_META_REG);
                  if(!log4){
                      printk("os chunk alloc failed at %s\n",__func__);
		  }
                  log4->event = VMA_REMOVE;
                  log4->address.index = get_vma_index(current,next);
                  log4->address.offset = 0;
                  log4->payload = NULL;
                  log4->size = 0;
                  list_add_tail(&log4->list, &cs->log);
		       ...
		       vm = vm->vm_next;
			      ...
			      vm = try_merge_vms(vm, prev_vma, 1); 
		              ...
		              vm = vm->vm_next;
                              ...
                              struct vm_area *new_vm_area = create_vm_area(caddr, vm->vm_end, prot,vm->is_nvm);
		              
			      vm->vm_end = caddr;
			      if(current->persistent){
                                  dprintk("creating VMA_ADD log at %s\n",__func__);
                                  struct saved_state* cs = get_saved_state(current);
                                  struct log_entry* log1 = (struct log_entry*)os_chunk_alloc(sizeof(struct log_entry),NVM_META_REG);
                                  if(!log1){
                                      printk("os chunk alloc failed at %s\n",__func__);
                                  ...
                                  log1->address.index = get_vma_index(current,vm);
                                  log1->address.offset = 0;
                                  log1->payload = (struct vm_area*)os_chunk_alloc(sizeof(struct vm_area),NVM_META_REG);
                                  if(!log1->payload){
                                      printk("os chunk alloc failed at %s\n",__func__);
				  }
                                  memcpy((char*)log1->payload, (char*)new_vm_area, sizeof(struct vm_area));
                                  log1->size = sizeof(struct vm_area);
                                  list_add_tail(&log1->list, &cs->log);
                                  clflush_multiline((u64)log1->payload, sizeof(struct vm_area));
                                  clflush_multiline((u64)log1, sizeof(struct log_entry));
			      ...
			      if(current->persistent){
                                  ...
                                  log2->address.index = get_vma_index(current,vm);
                                  log2->address.offset = 0;
                                  log2->payload = (struct vm_area*)os_chunk_alloc(sizeof(struct vm_area),NVM_META_REG);
                                  if(!log2->payload){
                                      printk("os chunk alloc failed at %s\n",__func__);
				  }
                                  memcpy((char*)log2->payload, (char*)new_vm_area, sizeof(struct vm_area));
                                  log2->size = sizeof(struct vm_area);
                                  list_add_tail(&log2->list, &cs->log);
                                  clflush_multiline((u64)log2->payload, sizeof(struct vm_area));
                                  clflush_multiline((u64)log2, sizeof(struct log_entry));
                                  clflush_multiline((u64)&cs->log, sizeof(struct list_head));
			      }
			      ...
			      vm = vm->vm_next;
                              ...
                              struct vm_area *new_vm_area = create_vm_area(caddr, end_addr, prot,vm->is_nvm);
                              struct vm_area *old_vm_area = create_vm_area(end_addr, vm->vm_end, vm->access_flags,vm->is_nvm);
			      ...
			      if(current->persistent){
                                  ...
                                  log3->address.index = get_vma_index(current,vm);
                                  log3->address.offset = 0;
                                  log3->payload = (struct vm_area*)os_chunk_alloc(sizeof(struct vm_area),NVM_META_REG);
                                  if(!log3->payload){
                                      printk("os chunk alloc failed at %s\n",__func__);
				  }
                                  memcpy((char*)log3->payload, (char*)old_vm_area, sizeof(struct vm_area));
                                  log3->size = sizeof(struct vm_area);
                                  list_add_tail(&log3->list, &cs->log);
                                  clflush_multiline((u64)log3->payload, sizeof(struct vm_area));
                                  clflush_multiline((u64)log3, sizeof(struct log_entry));
                                  ...
                                  log4->address.index = get_vma_index(current,vm);
                                  log4->address.offset = 0;
                                  log4->payload = (struct vm_area*)os_chunk_alloc(sizeof(struct vm_area),NVM_META_REG);
                                  if(!log4->payload){
                                      printk("os chunk alloc failed at %s\n",__func__);
				  }
                                  memcpy((char*)log4->payload, (char*)new_vm_area, sizeof(struct vm_area));
			      ...
			      vm->vm_next = new_vm_area;
                              vm->vm_end = caddr;
			      if(current->persistent){
                                  dprintk("creating VMA_CHANGE log at %s\n",__func__);
                                  struct log_entry* log5 = (struct log_entry*)os_chunk_alloc(sizeof(struct log_entry),NVM_META_REG);
                                  if(!log5){
                                      printk("os chunk alloc failed at %s\n",__func__);
				  }
                                  log5->event = VMA_CHANGE;
                                  log5->address.index = get_vma_index(current,vm);
                                  log5->address.offset = offsetof(struct vm_area, vm_end);
                                  log5->payload = (void*)os_chunk_alloc(sizeof(unsigned long),NVM_META_REG);
                                  if(!log5->payload){
                                      printk("os chunk alloc failed at %s\n",__func__);
				  }
                                  memcpy((char*)log5->payload, (char*)&vm->vm_end, sizeof(unsigned long));
                                  log5->size = sizeof(unsigned long);
                                  list_add_tail(&log5->list, &cs->log);
                                  clflush_multiline((u64)log5->payload, sizeof(unsigned long));
                                  clflush_multiline((u64)log5, sizeof(struct log_entry));
                                  clflush_multiline((u64)&cs->log, sizeof(struct list_head));
			      }
   ...
   while(vm && caddr < end_addr){
        if(vm->vm_start == caddr && end_addr >= vm->vm_end){  //The whole VMA is getting unmapped
		 ...
		 purge_mapping_range(current, caddr, vm->vm_end);
                 vm->vm_end = caddr;
                 if(current->persistent){
	            dprintk("creating VMA_CHANGE log2 at vm_area_unmap\n");
	            struct log_entry* log2 = (struct log_entry*)os_chunk_alloc(sizeof(struct log_entry),NVM_META_REG);
		    if(!log2){
		        printk("os chunk alloc failed at %s\n",__func__);
		    }
		 ...
		 vm->vm_start = end_addr;
                 if(current->persistent){
	            dprintk("creating VMA_CHANGE log3 at vm_area_unmap\n");
	            struct log_entry* log3 = (struct log_entry*)os_chunk_alloc(sizeof(struct log_entry),NVM_META_REG);
		    if(!log3){
		        printk("os chunk alloc failed at %s\n",__func__);
		    }
                    ...
                    log3->address.offset = offsetof(struct vm_area, vm_start);
                    log3->payload = (void*)os_chunk_alloc(sizeof(unsigned long),NVM_META_REG);
                    memcpy((char*)log3->payload, (char*)&vm->vm_start, sizeof(unsigned long));
	            log3->size = sizeof(unsigned long);
	            list_add_tail(&log3->list, &cs->log);
		    clflush_multiline((u64)log3->payload, sizeof(unsigned long));
                    clflush_multiline((u64)log3, sizeof(struct log_entry));
		 ...
		 if(current->persistent){ 
	            dprintk("creating VMA_ADD log4 at %s\n",__func__);
	            struct log_entry* log4 = (struct log_entry*)os_chunk_alloc(sizeof(struct log_entry),NVM_META_REG);
		    if(!log4){
		        printk("os chunk alloc failed at %s\n",__func__);
		    }
		    ...
		    clflush_multiline((u64)log4->payload, sizeof(struct vm_area));
	            ...
	            dprintk("creating VMA_CHANGE log5 at vm_area_unmap\n");
	            struct log_entry* log5 = (struct log_entry*)os_chunk_alloc(sizeof(struct log_entry),NVM_META_REG);
		    if(!log5){
		        printk("os chunk alloc failed at %s\n",__func__);
		    }
	            ...
	            log5->address.offset = offsetof(struct vm_area, vm_end);
	            log5->payload = (void*)os_chunk_alloc(sizeof(unsigned long),NVM_META_REG);
	            memcpy((char*)log5->payload, (char*)&vm->vm_end, sizeof(struct vm_area));
	            log5->size = sizeof(struct vm_area);
	            list_add_tail(&log5->list, &cs->log);
		    clflush_multiline((u64)log5->payload, sizeof(struct vm_area));
                    clflush_multiline((u64)log5, sizeof(struct log_entry));
